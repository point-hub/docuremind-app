...
import { useDeleteUserApi } from './delete.api'
...
const onDelete = async () => {
  ...
  const responseDelete = await deleteUserApi.send(id.value as string, errors.value)
  if (!responseDelete) {
    loadingState.value = false
    return
  }
  ...
  password.value = ''
  reason.value = ''
  toastRef?.value.toast(`Delete User "${name.value}" success`, { color: 'success' })
