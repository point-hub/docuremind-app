...
import { useGetOwnerApi } from './retrieve.api'
import { useUpdateOwnerApi } from './update.api'

const route = useRoute()
const router = useRouter()
const authStore = useAuthStore()
const getOwnerApi = useGetOwnerApi()
const updateOwnerApi = useUpdateOwnerApi()
...
const onUpdate = async () => {
  ...
  try {
    const response = await updateOwnerApi.send(route.params.id.toString(), form.data, form.errors)
    if (response) router.push('/owners')
  } catch (error) {
