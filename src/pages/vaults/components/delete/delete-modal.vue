...
import { useDeleteVaultApi } from './delete.api'
...
const onDelete = async () => {
  ...
  // start api call
  const deleteVaultApi = useDeleteVaultApi()
  try {
    await deleteVaultApi.send(id.value as string, errors.value)
    emit('deleted')
    password.value = ''
    reason.value = ''
    toastRef?.value.toast(`Delete Vault "${name.value}" success`, { color: 'success' })
  } catch (error) {
...
<template>
  <base-modal title="Delete Vault" :is-open="showModal" @on-close="toggleModal(false)">
